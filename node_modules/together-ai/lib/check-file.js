...
exports.validate_messages = validate_messages;
exports.validate_preference_openai = validate_preference_openai;
const node_unsafe_imports_1 = require("./node-unsafe-imports.js");
// Constants
const MIN_SAMPLES = 1;
...
(function (DatasetFormat) {
    DatasetFormat["GENERAL"] = "general";
    DatasetFormat["CONVERSATION"] = "conversation";
    DatasetFormat["INSTRUCTION"] = "instruction";
    DatasetFormat["PREFERENCE_OPENAI"] = "preference_openai";
})(DatasetFormat || (DatasetFormat = {}));
const JSONL_REQUIRED_COLUMNS_MAP = {
...
async function checkFile(file, purpose = 'fine-tune') {
    const filePath = (0, node_unsafe_imports_1.resolve)(file);
    const report_dict = {
    ...
    try {
        const stats = await (0, node_unsafe_imports_1.stat)(filePath);
        if (!stats.isFile()) {
        ...
        if (file_size > MAX_FILE_SIZE_GB * NUM_BYTES_IN_GB) {
            report_dict.message = `Maximum supported file size is ${MAX_FILE_SIZE_GB} GB. Found file with size of ${Math.round((file_size / NUM_BYTES_IN_GB) * 1000) / 1000} GB.`;
            report_dict.is_check_passed = false;
        }
        else if (file_size === 0) {
    ...
    const ext = (0, node_unsafe_imports_1.extname)(filePath);
    try {
        if (ext === '.jsonl') {
...
function validate_preference_openai(example, idx = 0) {
    ...
    if (example['input']['messages'].length > 0 &&
...
async function _check_utf8(file) {
    const report_dict = {};
    const content = await (0, node_unsafe_imports_1.readFile)(file);
    report_dict.utf8 = (0, node_unsafe_imports_1.isUtf8)(content);
    if (!report_dict.utf8) {
...
async function _check_csv(file, purpose) {
    ...
    const fileStream = (0, node_unsafe_imports_1.createReadStream)(file);
    const rl = node_unsafe_imports_1.readline.createInterface({
        input: fileStream,
        crlfDelay: Infinity,
    });
    let idx = -1;
...
async function _check_parquet(file, purpose) {
    ...
    try {
        ...
        const pkg = await Promise.resolve().then(() => __importStar(require('parquetjs')));
